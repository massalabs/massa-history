...
| No transfers indexed since 2026‑05‑17 18:30 UTC (period 4608114) | all three `massa-node` binaries rebuilt for MAIN.5.0 with default features → `NewTransfersInfoServer` `Unimplemented`; indexer warned once | nodes rebuilt with `--features execution-info` one at a time (idx3 → idx1 → idx2); live transfers flow again on all hosts; `[repair.reconstruct_transfers]` rebuilt 2 196 exact `Transaction` movements per host over the gap; `[repair.pull_parts]` copied the inter-rebuild window; `[repair.legacy_sub_transfers]` on idx1 recovered **27 471 ABI sub-transfers for 17 308 SC slots** from the legacy storer (19 874 DDB queries, 0 errors, ~5 min) and the other hosts pulled them with `require_sc_ops`. Block / endorsement rewards and operation fees for 2026‑05‑17 → 2026‑09‑11 remain **not recoverable** (no node can replay them and the legacy storer never stored them). |
| 5 357 slots stuck `Candidate` on every host (2026‑05‑08, 2026‑05‑18 22:26‑23:46 UTC — all nodes down together) | no indexer ever received the FINAL verdicts | targeted `[legacy_ddb]` import on idx1 (`min_period = 4557100`, `max_period = 4614700`): 6 797 slots from DDB in 90 s, 0 errors; idx2/idx3 converged through the mesh |
